...
class Admin extends DB_object
{
    ...
    protected static $db_table_field = array('name', 'username', 'email', 'password', 'role', 'status');
    ...
    public static function verify_admin($username, $password)
    {
        ...
        $sql = "SELECT * FROM " . self::$db_table . " WHERE username = '{$username}' OR email = '{$username}' AND status = 1";
